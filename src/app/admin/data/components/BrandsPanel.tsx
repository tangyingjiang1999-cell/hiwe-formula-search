    ...
    <div>
      ...
      <div className="overflow-x-auto rounded border border-gray-200 bg-white">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 text-left text-sm font-medium uppercase text-gray-500">
          ...
          <tbody>
            {brands.map((brand) => (
              <tr key={brand.id} className="border-b border-gray-100 last:border-0">
                ...
                <td className="px-4 py-3 text-sm font-semibold text-gray-900">{brand.name}</td>
                <td className="px-4 py-3 text-sm text-gray-600">{brand.region}</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => openEdit(brand)} className="mr-3 text-sm text-blue-600 hover:text-blue-800">编辑</button>
                  <button onClick={() => handleDelete(brand)} className="text-sm text-red-600 hover:text-red-800">删除</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="w-[90%] max-w-[400px] rounded-lg bg-white p-6 shadow-lg">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <select value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value as CarMake["region"] })} className="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm outline-none focus:border-[#0D9488]">
